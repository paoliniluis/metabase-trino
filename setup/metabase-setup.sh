...
curl -s -X POST http://$1/api/database -H 'Content-Type: application/json' --cookie "metabase.SESSION=$sessionToken" --data '{"engine":"starburst","name":"trino-server-system","details":{"host":"trino-server","port":8443,"catalog":"system","schema":null,"user":"admin","password":null,"ssl":false,"advanced-options":false},"is_full_sync":true}' && \
...
curl -s -X POST http://$1/api/database -H 'Content-Type: application/json' --cookie "metabase.SESSION=$sessionToken" --data '{"engine":"starburst","name":"trino-server-mysql","details":{"host":"trino-server","port":8443,"catalog":"mysql","schema":null,"user":"admin","password":null,"ssl":false,"advanced-options":false},"is_full_sync":true}' && \
