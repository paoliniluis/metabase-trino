...
if [ $(id -u) -ne 0 ]; then
    ...
    id metabase > /dev/null 2>&1
    user_exists=$?
    if [ $user_exists -ne 0 ]; then
        adduser --system --uid $MUID --ingroup metabase --no-create-home --disabled-login metabase
    fi
    ...
    if [ -f "${INITIAL_DB}" ]; then
        ...
        su metabase -s /bin/sh -c "exec /opt/java/openjdk/bin/java $JAVA_OPTS -jar /app/metabase.jar load-from-h2 ${INITIAL_DB%.mv.db} $@"
    ...
    # exec is here twice on purpose to  ensure that metabase runs as PID 1 (the init process)
    # and thus receives signals sent to the container. This allows it to shutdown cleanly on exit
    exec su metabase -s /bin/sh -c "exec /opt/java/openjdk/bin/java $JAVA_OPTS -jar /app/metabase.jar $@"
fi